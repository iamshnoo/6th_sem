...
int main()
{
	...
	int i;
	char buf[20];
	char search_table[10][10] = {"success", "above", "easy", "accept", "enter", "hot", "up", "left", "healthy", "summer"};
	char opp_table[10][10] = {"failure", "below", "hard", "reject", "exit", "cold", "down", "right", "sick", "winter"};
	...
	*/
	while (1) {
		...
		*/
	    ...
	    for (int i = 0; i<10; i++)
			{
				if (strcmp(buf, search_table[i]) == 0)
				{
					strcpy(buf, opp_table[i]);
					found = 1;
				}
			}
